(function() {
  ...
  const data = [
    {x: 0, y1: 5000, y2: 128154, title: {y1: '6000', y2: '138 154'}},
    {x: 1, y1: 6000, y2: 138154, title: {y1: '6000', y2: '138 154'}},
    {x: 2, y1: 8500, y2: 169162, title: {y1: '8500', y2: '189 162'}},
    {x: 3, y1: 12700, y2: 210000, title: {y1: '21 500', y2: '477 500'}},
    {x: 4, y1: 19500, y2: 250000, title: {y1: '45 000', y2: '999 320'}}
  ]
  ...
  function createLine(dataLine, color, title) {
    ...
    let dots = svg.selectAll("dot")
      ...
      .append('text')
      .text(d => d.title)
      .attr('style', 'color:'+color+'; font-size: 36px; font-weight: 400; font-family: "Gotham Pro", Arial, sans-serif');

    let titles = svg.selectAll("dot")
      .data(dataLine)
      .enter()
      .filter((item, i) => {
        return (i > 0 && i < 5);
...
(function() {
  ...
  let margin = 20;
	...
	d3.select(window).on('resize.line', () => {
	  ...
	  svg.select('.line')
	  	.attr("d", valueline);
	  svg.select('.line-progress')
	  	.attr("d", valueline);
	  svg.select('.line-area')
	  	.attr("d", area);
	  svg.select('.dot')
      .attr('cx', d => x(extData[extData.length-1].x))
      .attr('cy', d => y(extData[extData.length-1].y))
...
(function() {
  ...
  function drawHistogram(info) {
    ...
    svg.selectAll('.tick text')
      ...
      .attr('dy', height + 10)
      .attr('style', 'font-size: 16px; font-family: "Gotham Pro", Arial, sans-serif; font-weight: 300; fill: #262729;')
      .call(wrap, x.bandwidth() - 20);

    svg.selectAll('.x-axis .tick')
      .append('text')
      .attr('class', 'text')
      .text((d, i)=> '$' + info[i].value)
      .attr('dy', (d, i) => height - hs[i] - 10)
      .attr('style', 'font-size: 24px; font-family: "Gotham Pro", Arial, sans-serif; font-weight: 300; fill: #262729;')
  }
  ...
  function updateHistogram(info) {
    ...
    bar.exit().remove();
    bar.data(info).enter().append('rect')
      .merge(bar)
      .attr('fill', function(d, i) {
        return (i === 2) ? '#ea4a3e' : '#41aef1';
      })
